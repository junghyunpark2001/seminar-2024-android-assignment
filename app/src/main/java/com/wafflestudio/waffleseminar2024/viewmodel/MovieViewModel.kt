...
class MovieViewModel(private val repository: MovieRepository) : ViewModel() {
    ...
    private suspend fun updateFavoriteMovieTitles() {
        ...
        // Room DB에서 찜한 영화 ID 목록을 가져와 Movie 객체로 변환하는 메서드
    fun updateFavoriteMovies() {
        viewModelScope.launch {
            val movieIds = repository.getAllFavoriteMovieIds() // Room DB에서 영화 ID 가져오기
            val movies = movieIds.mapNotNull { id ->
                withContext(Dispatchers.IO) {
                    repository.getMovieById(id) // 영화 정보를 가져와 Movie 객체 생성
                }
            }
            _favoriteMovies.postValue(movies) // LiveData 업데이트
        }
    }
}
//package com.wafflestudio.waffleseminar2024.viewmodel
...
//    val searchResults: LiveData<List<Movie>> get() = _searchResults
//
//    private val _favoriteMovieTitles = MutableLiveData<List<Movie>>()
//    val favoriteMovieTitles: LiveData<List<Movie>> get() = _favoriteMovieTitles
//
//    private val _favoriteMovies = MutableLiveData<List<Movie>>()
//
//    // 외부에 LiveData 형태로 노출합니다.
//    val favoriteMovies: LiveData<List<Movie>> get() = _favoriteMovies
//
//    fun fetchMovieDetails(id: Int) {
//        viewModelScope.launch {
